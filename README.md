...
Eredivisie Fantasy fantasy game data, fetched directly from the official fantasy API at
`https://fantasy.espngoal.nl/api/`.
...
.
├── .github/workflows/
│   └── eredivisie-fetch.yml         # scheduled daily fetch (1am UTC, retries 2am/3am)
├── scripts/
...
│       ├── eredivisie-bootstrap_{season}.json
│       ├── eredivisie-fixtures_{season}.json
│       ├── eredivisie-dream-team_{season}.json
│       ├── eredivisie-regions_{season}.json
...
└── latest/                          # always the most recent data, season-agnostic
    ├── eredivisie-bootstrap.json
    ├── eredivisie-fixtures.json
...
| File | Description |
|---|---|
| `data/{season}/eredivisie-bootstrap_{season}.json` | Players, teams, gameweeks, game settings |
| `data/{season}/eredivisie-fixtures_{season}.json` | All season fixtures with scores |
...
| `data/{season}/eredivisie-dream-team_{season}.json` | Season dream team (updated each GW) |
| `data/{season}/eredivisie-set-piece-notes_{season}.json` | Set piece taker notes (if the league API provides them) |
| `data/{season}/eredivisie-regions_{season}.json` | Region/nationality reference (if the league API provides it) |
...
| `data/{season}/csv/teams.csv` | All teams listed in the bootstrap (18 compete in Eredivisie; the platform may list extra historical entries) |
...
| File | Description |
|---|---|
| `latest/eredivisie-bootstrap.json` | Current bootstrap |
| `latest/eredivisie-fixtures.json` | Current fixtures |
...
# Latest bootstrap
url = "https://raw.githubusercontent.com/TopMarx/eredivisie/main/latest/eredivisie-bootstrap.json"
...
```
https://raw.githubusercontent.com/TopMarx/eredivisie/main/data/{season}/eredivisie-bootstrap_{season}.json
https://raw.githubusercontent.com/TopMarx/eredivisie/main/latest/eredivisie-bootstrap.json
```

### Clone the repo

```
git clone https://github.com/TopMarx/eredivisie.git
```
